...
function updateRecentEvents(events) {
  ...
  if (!events || events.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="4" style="text-align: center;">Nenhum evento</td></tr>';
    return;
  }
...
function updateEventsTable(events) {
  ...
  if (!events || events.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="5" style="text-align: center;">Nenhum evento</td></tr>';
    return;
  }
  tbody.innerHTML = events
    .map(
...
function updatePresenceTable(presence) {
  ...
  if (!presence || presence.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="4" style="text-align: center;">Nenhuma pessoa dentro</td></tr>';
    return;
  }
  tbody.innerHTML = presence
    .map(
      (p) => `
        <tr>
            <td>${p.username || "-"}</td>
...
async function updateUsersTable() {
  ...
  if (!usersCache || usersCache.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="4" style="text-align: center;">Nenhum usuário</td></tr>';
    return;
  }
...
async function updateCellsGrid() {
  ...
  if (!cellsCache || cellsCache.length === 0) {
    container.innerHTML =
      '<p style="text-align: center; padding: 2rem;">Nenhuma célula cadastrada</p>';
    return;
  }
  container.innerHTML = cellsCache
    .map(
      (cell) => `
        <div class="card">
...
async function updateDevicesGrid() {
  ...
  try {
    ...
    if (!devices || devices.length === 0) {
      container.innerHTML =
        '<p style="text-align: center; padding: 2rem;">Nenhum dispositivo cadastrado</p>';
      return;
    }
    ...
    container.innerHTML =
      '<p style="color: var(--danger); text-align: center;">Erro ao carregar dispositivos</p>';
  }
}
...
function openAddDeviceModal() {
  ...
  document.querySelector("#deviceModal .modal-header h2").textContent =
    "Novo Dispositivo Intelbras";
  populateCellSelect();
...
function openAddCellModal() {
  ...
  document.querySelector("#cellModal .modal-header h2").textContent =
    "Nova Célula";
...
async function handleAddCell(event) {
  ...
  const payload = {
    ...
    plc_start_byte:
      parseInt(document.getElementById("cellPlcStart").value) || 0,
  };
  try {
    if (editingCellId) {
...
async function deleteDevice(id) {
  if (confirm("Deletar dispositivo?")) {
    await ApiClient.delete(`/intelbras-devices/${id}`);
    updateDevicesGrid();
  }
}
...
async function openUserModal() {
  try {
    ...
    if (userCellSelect) {
      userCellSelect.innerHTML = cells
    ...
    if (targetDevicesSelect) {
      targetDevicesSelect.innerHTML = devices
        .map(
          (d) => `<option value="${d.id}">${d.name} (${d.ip_address})</option>`,
        )
        .join("");
    }
...
function closeUserModal() {
  ...
  stopCamera(); // <--- ADICIONE ESTA LINHA AQUI
...
if (userForm) {
  userForm.addEventListener("submit", async (e) => {
    ...
    const selectedDevices = selectElement
      ? Array.from(selectElement.selectedOptions).map((option) => option.value)
      : [];

    if (selectedDevices.length === 0)
      return showAlert("Selecione pelo menos um dispositivo!", "warning");
    ...
    try {
      showAlert(
        `⏳ Enviando biometria para ${selectedDevices.length} dispositivo(s)...`,
        "info",
      );
  ...
  try {
    ...
    if (tbody) {
      if (permissions.length === 0) {
        tbody.innerHTML =
          '<tr><td colspan="3" style="text-align: center;">Nenhuma permissão cadastrada</td></tr>';
        return;
      }
...
async function revokePermission(userId, cellId) {
  if (
    confirm(
      "Tem certeza que deseja revogar o acesso deste usuário a esta célula?",
    )
  ) {
    try {
...
async function startCamera() {
  ...
  try {
    // Solicita permissão e acesso à câmera web
    videoStream = await navigator.mediaDevices.getUserMedia({ video: true });
    video.srcObject = videoStream;
    cameraPreview.style.display = "block"; // Mostra a área da câmera
  } catch (err) {
    console.error("Erro ao acessar a câmera: ", err);
    showAlert(
      "❌ Não foi possível acessar a câmera. Verifique se o site tem permissão e está usando HTTPS (ou localhost).",
      "danger",
    );
  }
}
...
function capturePhoto() {
  ...
  // Define o tamanho da foto baseado no tamanho do vídeo
  canvas.width = video.videoWidth;
  canvas.height = video.videoHeight;

  // "Bate a foto": Desenha o frame atual do vídeo no canvas
  const context = canvas.getContext("2d");
  context.drawImage(video, 0, 0, canvas.width, canvas.height);

  // Converte a imagem gerada para formato Base64 (JPEG)
  ...
  // TRUQUE DE MÁGICA: Converte o Base64 em um arquivo (File) e coloca dentro do <input type="file">
  fetch(dataUrl)
    .then((res) => res.blob())
    .then((blob) => {
  ...
  // Desliga a câmera e avisa o usuário
  stopCamera();
